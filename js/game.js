import {
  ...
  ROLE_LABELS,
  TIMED_SECONDS
} from './constants.js?v=4.0.3';
import { filterSpells, getChampions, getDataVersion, getSpells, safeImage } from './data.js?v=4.0.3';
import {
  addReviewMistake,
  addXp,
...
function focusGuessInput({ retry = true, scroll = true } = {}) {
  ...
  // iOS è più affidabile con focus() semplice, eseguito direttamente
  // nello stesso touch dell'utente. Le opzioni di focus possono essere
  // accettate senza errore ma non far comparire la tastiera.
  if (isIOSDevice()) {
...
export function startGame(mode, suppliedConfig = null, directKeyboard = false) {
...
function loadRound({ directKeyboard = false } = {}) {
  clearTimeout(state.nextTimeoutId);
  clearTimeout(state.flashTimeoutId);
  ...
  // Su iPhone il focus deve essere eseguito immediatamente nello stesso
  // touch dell'utente. Lo facciamo prima di immagini, HUD e animazioni.
  if (directKeyboard && isIOSDevice()) {
...
function checkAbilitySlot(slot, directKeyboard = false) {
  ...
  if (slot === state.currentSpell.slot) {
    ...
    if (directKeyboard && isIOSDevice()) {
      const solved = `${state.currentSpell.champ} · ${state.currentSpell.slot} · ${state.currentSpell.spellName}`;
      updateHud();
      advanceRound(true);
      if (hasAnotherRound) toast(`Corretta: ${solved}`, 'correct', 2200);
      return;
    }
...
function advanceRound(directKeyboard = false) {
...
export function initializeGameUi() {
  ...
  bindTouchSafeAction(dom['practice-btn'], (_event, direct) => startGame('practice', null, direct));
  bindTouchSafeAction(dom['hardcore-btn'], (_event, direct) => startGame('hardcore', null, direct));
  bindTouchSafeAction(dom['timed-btn'], (_event, direct) => startGame('timed', null, direct));
  bindTouchSafeAction(dom['daily-btn'], (_event, direct) => startGame('daily', null, direct));
  bindTouchSafeAction(dom['review-btn'], (_event, direct) => startGame('review', null, direct));
  ...
  bindTouchSafeAction(dom['next-spell-btn'], (_event, direct) => advanceRound(direct));
  ...
  bindTouchSafeAction(dom['replay-btn'], (_event, direct) => startGame(state.lastConfig.mode, state.lastConfig, direct));
  bindTouchSafeAction(dom['review-after-btn'], (_event, direct) => startGame('review', null, direct));
  ...
  document.querySelectorAll('.spell-btn').forEach(button => {
    bindTouchSafeAction(button, (_event, direct) => checkAbilitySlot(button.dataset.slot, direct));
  });
